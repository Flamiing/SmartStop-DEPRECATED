...
if [[ $1 == '-t' ]] || [[ $1 == '-T' ]]; then 
    ...
    if [[ $2 == "true" ]]; then 
        ...
        sudo sed -i "s/WINTERMINAL=.*/WINTERMINAL=$2/g" /etc/Smart\ Stop/stop.conf
...
# Checks if user wants to change the DISTRO
    ...
    if [ ! -z $2 ]; then 
        # Modifyes the config file with the user preference
        sudo sed -i "s/DISTRO=.*/DISTRO=$2/g" /etc/Smart\ Stop/stop.conf
...
# Checks if user wants to change the IDE
    ...
    if [ ! -z $2 ]; then 
        # Modifyes the config file with the user preference
        sudo sed -i "s/EDITOR=.*/EDITOR=$2/g" /etc/Smart\ Stop/stop.conf
